...
class PayfortIntegration
{
    ...
    public function getRedirectionData($paymentMethod) {
        ...
        $postData = array(
            ...
            'return_url'          => $this->getUrl('wp-content/plugins/custompayfort/payment/?pl=r&r=processResponse'),
        );
    ...
    {
        $merchantReference = $this->generateMerchantReference();
        $returnUrl = $this->getUrl('wp-content/plugins/custompayfort/payment/?pl=r&r=merchantPageReturn');
     
       /* if(isset($_GET['3ds']) && $_GET['3ds'] == 'no') {
            $returnUrl = $this->getUrl('wp-content/plugins/custompayfort/payment/?p=r&r=merchantPageReturn&3ds=no');
        }*/
        ...
        #echo $iframeParams['signature'] . '<br>';
    ...
    public function processResponse()
    {
        ...
        if(!$success) {
            $p = $params;
            $p['error_msg'] = $reason;
            $return_url = $this->getUrl('payment/?pl=error&'.http_build_query($p));
            
        }
        else{
            $return_url = $this->getUrl('payment/?pl=success&'.http_build_query($params));
        }
    ...
    public function calculateSignature($arrData, $signType = 'request')
    {
        ...
        $debugMsg = "Custom $signType $signature \n".print_r($arrData, 1);
        $this->log($debugMsg);
        return $signature;
    }
